...
def knn_impute(dist1, dist2, mask, n_neighbors=10):
    """
    使用 KNN 方法填补相似性矩阵中的缺失数据。
    
    参数：
    dist1 (np.ndarray): 第一个相似性矩阵。
    dist2 (np.ndarray): 第二个相似性矩阵。
    mask (np.ndarray): 二进制掩码矩阵，指示数据的有效性（True 表示有效，False 表示缺失）。
    n_neighbors (int): KNN 中的邻居数量。
    
    返回：
    np.ndarray, np.ndarray: 填补后的相似性矩阵 dist1 和 dist2。
    """
    # 将掩码应用于矩阵
    combined_mask = mask[:, 0] & mask[:, 1]
    combined_dist = np.where(combined_mask[:, None], dist1, np.nan)
    
    # 使用 KNN 填补缺失数据
    imputer = KNNImputer(n_neighbors=n_neighbors)
    filled_dist1 = imputer.fit_transform(combined_dist)
    
    # 同样处理 dist2
    combined_dist = np.where(combined_mask[:, None], dist2, np.nan)
    filled_dist2 = imputer.fit_transform(combined_dist)
    
    return filled_dist1, filled_dist2

def combined_impute(dist1, dist2, mask, n_neighbors=10):
    """
    结合关系转移和 KNN 方法填补缺失数据。
    ...
    # KNN 填补
    def knn_fill(matrix, mask, n_neighbors):
        # 创建一个 KNNImputer 实例
        imputer = KNNImputer(n_neighbors=n_neighbors)
        # 将掩码应用于矩阵
        matrix_filled = np.where(mask, matrix, np.nan)
        # 使用 KNN 填补缺失数据
        return imputer.fit_transform(matrix_filled)
    
    filled_dist1 = knn_fill(dist1, mask[:, 0], n_neighbors)
    filled_dist2 = knn_fill(dist2, mask[:, 1], n_neighbors)
...
def get_miss_adjacency(features1, features2, mask, n, topk=10):
    ...
    dist1, dist2 = combined_impute(dist1, dist2, mask)
