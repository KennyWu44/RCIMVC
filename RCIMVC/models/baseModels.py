...
class AttentionLayer_3v(nn.Module):
    def __init__(self, latent_dim):
        super(AttentionLayer_3v, self).__init__()
        self._latent_dim = latent_dim
        self.mlp = nn.Sequential(
            nn.Linear(self._latent_dim * 3, self._latent_dim * 3),
    ...
    def forward(self, h1, h2, h3, tau=3):
...
class AttentionLayer_4v(nn.Module):
    def __init__(self, latent_dim):
        super(AttentionLayer_4v, self).__init__()
        self._latent_dim = latent_dim
        self.mlp = nn.Sequential(
            nn.Linear(self._latent_dim * 4, self._latent_dim * 4),
            nn.BatchNorm1d(self._latent_dim * 4),
            nn.ReLU(),
            nn.Linear(self._latent_dim * 4, self._latent_dim * 4),
            nn.BatchNorm1d(self._latent_dim * 4),
            nn.ReLU(),
        )
        self.output_layer = nn.Linear(self._latent_dim * 4, 4, bias=True)

    def forward(self, h1, h2, h3, h4, tau=4):
        ...
        # weights = torch.mean(e, dim=0)
        # h = weights[0] * h1 + weights[1] * h2
        h = e[:, 0].unsqueeze(1) * h1 + e[:, 1].unsqueeze(1) * h2 + e[:, 2].unsqueeze(1) * h3 + e[:, 3].unsqueeze(1) * h4
        return h

class AttentionLayer_6v(nn.Module):
    def __init__(self, latent_dim):
        super(AttentionLayer_6v, self).__init__()
        self._latent_dim = latent_dim
        self.mlp = nn.Sequential(
            nn.Linear(self._latent_dim * 6, self._latent_dim * 6),
            nn.BatchNorm1d(self._latent_dim * 6),
            nn.ReLU(),
            nn.Linear(self._latent_dim * 6, self._latent_dim * 6),
            nn.BatchNorm1d(self._latent_dim * 6),
            nn.ReLU(),
        )
        self.output_layer = nn.Linear(self._latent_dim * 6, 6, bias=True)

    def forward(self, h1, h2, h3, h4, h5, h6, tau=6):
        ...
        # weights = torch.mean(e, dim=0)
        # h = weights[0] * h1 + weights[1] * h2
        h = e[:, 0].unsqueeze(1) * h1 + e[:, 1].unsqueeze(1) * h2 + e[:, 2].unsqueeze(1) * h3 + e[:, 3].unsqueeze(1) * h4 + e[:, 4].unsqueeze(1) * h5 + e[:, 5].unsqueeze(1) * h6
        return h
