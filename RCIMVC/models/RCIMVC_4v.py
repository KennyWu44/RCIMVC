...
class RCIMVC(nn.Module):
    def __init__(self, config):
        ...
        self._input_dim4 = config['Autoencoder']['gcnEncoder4'][0]
        # self._input_dim5 = config['Autoencoder']['gcnEncoder5'][0]
        # self._input_dim6 = config['Autoencoder']['gcnEncoder6'][0]
        ...
        self.gcnEncoder4 = GraphEncoder(config['Autoencoder']['gcnEncoder4'], 'relu', True)
        # self.gcnEncoder5 = GraphEncoder(config['Autoencoder']['gcnEncoder5'], 'relu', True)
        # self.gcnEncoder6 = GraphEncoder(config['Autoencoder']['gcnEncoder6'], 'relu', True)
        ...
        self.instance_projector4 = InstanceProject(self._latent_dim)
        # self.instance_projector5 = InstanceProject(self._latent_dim)
        # self.instance_projector6 = InstanceProject(self._latent_dim)

        self.cluster = ClusterProject(self._latent_dim, self._n_clusters)
        self.fusion = AttentionLayer_4v(self._latent_dim)
    ...
    def forward(self, x1, x2, x3, x4, adj1, adj2, adj3, adj4):
        ...
        h4 = self.gcnEncoder4(x4, adj4)
        # h5 = self.gcnEncoder5(x5, adj5)
        # h6 = self.gcnEncoder6(x6, adj6)
        ...
        z4 = normalize(self.instance_projector4(h4), dim=1)
        # z5 = normalize(self.instance_projector5(h5), dim=1)
        # z6 = normalize(self.instance_projector6(h6), dim=1)

        y1, p1 = self.cluster(h1)
        y2, p2 = self.cluster(h2)
        y3, p3 = self.cluster(h3)
        y4, p4 = self.cluster(h4)
        # y5, p5 = self.cluster(h5)
        # y6, p6 = self.cluster(h6)

        return h1, h2, h3, h4, z1, z2, z3, z4, y1, y2, y3, y4, p1, p2, p3, p4
    ...
    def run_train(self, x_train, Y_list, adj, optimizer, logger, accumulated_metrics, device):
        ...
        for k in range(epochs):
            h1, h2, h3, h4, z1, z2, z3, z4, y1, y2, y3, y4, p1, p2, p3, p4,  = self(x_train[0], x_train[1], x_train[2], x_train[3], adj[0], adj[1], adj[2], adj[3])
            if attention:
                h = self.fusion(h1, h2, h3, h4)
            else:
                h = (h1 + h2 + h3 + h4) / 4

            # cluster contrastive loss
            cluster_loss = (criterion_cluster(y1, y2) + criterion_cluster(y1, y3) + criterion_cluster(y1, y4) + criterion_cluster(y2, y3) + criterion_cluster(y2, y4) + criterion_cluster(y3, y4)) / 6
            ...
            y_max = torch.maximum(y_max, y4)
            # y_max = torch.maximum(y_max, y5)
            # y_max = torch.maximum(y_max, y6)
            ...
            q_memo = torch.cat((y1, y2, y3, y4), dim=0)
            q_mean = (y1+y2+y3+y4)/len(q_memo)
