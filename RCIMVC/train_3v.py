...
import collections
import warnings
from models.RCIMVC_3v import RCIMVC
    ...
    for flag in [11]:
        ...
        # for ms in [0.7]:
            for k in range(10, 10 + 1):
                ...
                # mask the data
                np.random.seed(2023)
                mask = get_mask(x1_train_raw.shape[0], config['missing_rate'], 3)
                ...
                adj32, _, adj12, _ = get_miss_adjacency(x3_train, x1_train, mask, x3_miss.shape[0], topk=config['topk'])             
                adj1 = adj11 + adj12
                adj2 = adj21 + adj22
                adj3 = adj31 + adj32
                adj1 = adj1.cuda()
                ...
                for data_seed in range(1, test_time + 1):
                    ...
                    model.to(device)
                    # Training
                    acc, nmi, ari, H, H1, H2, Q = model.run_train([x1_train, x2_train, x3_train], Y_list, [adj1, adj2, adj3],
